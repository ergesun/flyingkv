...
SndMessage::SndMessage(sys::MemPool *mp, net_peer_info_t &&peerInfo) : Message(mp) {
#ifdef WITH_MSG_ID
    m_header.id = GetNewId();
#endif
...
#ifdef WITH_MSG_ID
SndMessage::SndMessage(sys::MemPool *mp, net_peer_info_t &&peerInfo, Id id) : Message(mp) {
    m_header.id = id;
    m_peerInfo = std::move(peerInfo);
}
#endif
