...
namespace minikv {
namespace server {
RaftConsensus::RaftConsensus() : m_random(sys::Random::Range(FLAGS_start_election_rand_latency_low,
...
void RaftConsensus::OnMessageSent(rpc::ARpcClient::SentRet &&sentRet) {
    ...
    sys::WriteLock wl(&m_sentMsgsIdRwMtx);
...
void RaftConsensus::prepare_new_election_env() {
    {
        sys::WriteLock rl(&m_sentMsgsIdRwMtx);
        m_sentMsgsId.clear();
    }

    sys::WriteLock wl(&m_envRwLock);
...
inline void RaftConsensus::subscribe_leader_hb_timer_tick() {
    ...
    sys::cctime interval(FLAGS_raft_election_interval, latency * 1000 * 1000);
...
inline void RaftConsensus::subscribe_request_vote_timeout_tick() {
    sys::cctime interval(m_iRequestVoteTimeoutInterval, 0);
...
bool RaftConsensus::is_valid_msg_id(const net::RcvMessage *rm) {
    ...
    switch (nrt) {
        ...
        case NodeRoleType::Candidate:{
            sys::ReadLock rl(&m_sentMsgsIdRwMtx);
